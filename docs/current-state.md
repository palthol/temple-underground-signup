...
No production writes were performed. The deployed Express hostname and runtime environment
were not available, so code/schema compatibility is verified but deployed route health is
not.
...
- CORS defaults to `*` when `ALLOWED_ORIGIN` is absent.
